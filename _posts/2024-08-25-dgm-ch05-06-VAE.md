...
- 여러 간단한 모델들을 합쳐, 하나의 복잡한 모델을 표현하자.
  $p(x)=\sum_z\,p(x,z)=\sum_z\,p(z)p(x\|z)=\sum_{k=1}^Kp(z=k)N(x;\mu_k,\Sigma_k)$
...
\begin{aligned}
P_\theta(x) = \sum_zp_\theta(x,z)=\|Z\|\sum_{z \in Z}\frac{1}{\|Z\|}p_\theta(x,z) = \|Z\| E_{z\sim Uniform(Z)}[P_\theta(x,z)]
\end{aligned}
...
- $log(E_{z \sim q(z)}[f(z)]) = log(\sum_zq(z)f(z)) \geq \sum_zq(z)\,log\,f(z) = E_{z_q(z)}[log\,f(z)]$
  Choosing $f(z)=\frac{p_\theta(x,z)}{q(z)}$
  \begin{aligned}
...
\begin{aligned}
\mathcal{L}(x^i;\theta,\Phi)=E_{q_\Phi(z\|x)}[log\,p(z,x;\theta) - log\,q_\Phi(z\|x)]
\end{aligned}
...
\begin{eqnarray}
\mathcal{L}(x^i;\theta,\Phi) & = &E_{q_\Phi(z\|x)}[log\,p(z,x;\theta) - log\,q_\Phi(z\|x)]
\newline & = & E_{q_\Phi(z\|x)}[log\,p(z,x;\theta) - log\,p(z) + log\,p(z) - log\,q_\Phi(z\|x)]
\newline & = & $E_{q_\Phi(z\|x)}[log(p(x\|z;\theta))] - D_{KL}(q_\Phi(z\|x)\|\|p(z))
\end{eqnarray}
...
- KL Divergence에 대입하면,
\begin{eqnarray}
\nonumber D_{KL}​(q(z)\|\|p(z))&=&E_{q(z)}[log\frac{\frac{1}{\sqrt{2\pi\sigma^2}} exp(-\frac{(z-\mu)^2}{2\sigma^2})​}{\frac{1}{\sqrt{2\pi}} exp(-\frac{z^2}{2})}]
...
- 따라서 전체를 결합하면,
\begin{eqnarray}
D_{KL}​(q(z)\|\|p(z)) &=& -\frac{1}{2}log(\sigma^2) -\frac{1}{2} + \frac{1}{2}(\mu^2 +\sigma^2) \nonumber
