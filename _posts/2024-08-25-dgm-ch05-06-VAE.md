...
\\ &= \sum_z P_\theta(z\|x; \theta)log\, P_\theta(x;\theta)
\\ &= log\, (P_\theta(x; \theta))
\end{aligned}
$$

> 참고, ($\sum_z P_\theta(z\|x; \theta) = 1$) 이므로
...
\\ &= E_{z \sim q(z)}[log\frac{P_\theta(x)P_\theta(z\|x)}{q(z)}]\,...\,\text{로그 성질로 분리}
...
\\ & q(z)와\,P_\theta(z\|x)\,\text{사이의 KL-divergence는 }E_{z \sim q(z)}[log\frac{q(z)}{P_\theta(z\|x)}]
\\ &\text{따라서, } D_{KL}(q(z) \|\| {P_\theta(z\|x)}) = E_{z \sim q(z)}[log\frac{q(z)}{P_\theta(z\|x)}]\text{ 이고, ELBO 수식을 다시 정리하면,}
\\ &log\,P_\theta(x) - D_{KL}(q(z) \|\| {P_\theta(z\|x)})
\end{aligned}
$$
