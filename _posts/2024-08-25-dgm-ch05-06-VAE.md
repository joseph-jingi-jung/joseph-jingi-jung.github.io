...
- 여러 간단한 모델들을 합쳐, 하나의 복잡한 모델을 표현하자.
  $$ p(x) = \sum*zp(x,z) =\sum_zp(z)p(x|z) = \sum*{k=1}^Kp(z=k)N(x;\mu_k,\Sigma_k)$$
- 이 수식에서 p(z)는 Categorial distribution.
- 뒤의 Normal 분포는 pre-defined된 look up table에서 선택 한다고 볼 수 있다.
  ![[Pasted image 20240823160134.png]]
...
- $z$를 Gaussian에서 뽑고, 그 $z$를 Neural Network에 통과시켜서 $\mu, \sigma$ 를 구함.
  - $z\sim N(0,I)$
  - $p(x|z) = N(\mu_\theta(z), \Sigma_\theta(z))$ where $\mu_\theta(z), \Sigma_\theta(z)$ are neural networks
    - $N(\mu_\theta(z), \Sigma_\theta(z))$ 의 차원은 $x$의 차원에 맞춰짐.
  - $p(x|z)$는 간단한 모델이나, $z$에 대해 marginal 인 $p(x)$는 매우 복잡함

### Variational autoencoder with joint distribution

- 만약 모델이 Joint distribution임을 가정한다면, 관찰된 training data point $\bar{x}$ 의 확률 $p(X=\bar{x};\theta)$를 구하려면, $$ \int_zp(X=\bar{x}, Z = z; \theta)dz = \int_zp(\bar{x},z;\theta)dz$$
- 관찰된 Variable $X$에 대한 Dataset $D$와 관찰되지 않은 Variable $Z$가 주어짐.
- 이를 Maximum likelihood learning 을 한다면, $p(x) = \sum_zp(x,z)$ 이므로, $$log\prod_{x \in D}p(x;\theta) = \sum_{x \in D}log\,p(x;\theta) = \sum_{x \in D}log\, \sum_zp(x,z;\theta)$$
...
- Navie Monte Carlo를 이용하여, z를 uniform하게 뽑고, 기댓값을 sample average를 통해서 추정.$$ P*\theta(x) = \sum_zp*\theta(x,z)=|Z|\sum*{z \in Z}\frac{1}{|Z|}p*\theta(x,z) = |Z| E*{z\sim Uniform(Z)}[P*\theta(x,z)]$$
...
- $z$를 임의의 분포 $q$ 에서 뽑고, 이를 이용해서 기댓값을 추정해보자.$$ P*\theta(x) = \sum_zp*\theta(x,z)=\sum*{z \in Z}\frac{q(z)}{q(z)}p*\theta(x,z)=\sum*{z \in Z}q(z)\frac{p*\theta(x,z)}{q(z)} = E*{z\sim q(Z)}[\frac{P*\theta(x,z)}{q(z)}]$$
...
- 따라서, $$ log(P*\theta(x)) = log(\sum_zp*\theta(x,z))=log(\sum*{z \in Z}\frac{q(z)}{q(z)}p*\theta(x,z))=log(E*{z\sim q(Z)}[\frac{P*\theta(x,z)}{q(z)}])$$
...
- $log(E_{z \sim q(z)}[f(z)]) = log(\sum_zq(z)f(z)) \geq \sum_zq(z)\,log\,f(z) = E_{z_q(z)}[log\,f(z)]$
  Choosing $f(z)=\frac{p_\theta(x,z)}{q(z)}$ $$log(E_{z\sim q(Z)}[\frac{P_\theta(x,z)}{q(z)}]) \geq E_{z \sim q(z)}[log(\frac{p_\theta(x,z)}{q(z)})]$$
  ...
  - $log(P_\theta(x)) \geq \sum_z q(z) log (\frac{P_\theta (x, z)}{q(z)})$
    $= \sum_z q(z)log\,P_\theta(x,z) - \sum_z q(z)log\,q(z)$ (두번째 텀은 Entropy $H(q)$ of $q$)
    $= \sum_z q(z)log\,P_\theta(x,z) + H(q)$
- 여기서 $q(z)=P_\theta(z|x;\theta)$ 라고 가정한다면, Lower bound는 아래와 같이 변경된다
  - $\sum_z p_\theta(z|x; \theta) log \frac{P_\theta(x,z; \theta)}{P_\theta(z|x; \theta)} = \sum_z P_\theta(z|x; \theta)log \frac{P_\theta(z|x; \theta)P_\theta(x;\theta)}{P_\theta(z|x; \theta)}$
    $= \sum_z P_\theta(z|x; \theta)log\, P_\theta(x;\theta)$ 여기서, ($\sum_z P_\theta(z|x; \theta) = 1$) 이므로
    $= log\, (P_\theta(x; \theta))$
- 따라서, bound가 equal로 tight하게 바뀜을 알 수 있다.
- 또한 $ELBO(q)$를 KL-Divergence 관점에서 보려면,
  - $ELBO(q) = E_{z \sim q(z)}[log\frac{P_\theta(x,z)}{q(z)}]$ 가 되고, 이를 위처럼 분리해보면,
    $= E_{z \sim q(z)}[log\frac{P_\theta(x)P_\theta(z|x)}{q(z)}]$ 이고, 이를 로그 성질로 분리하면,
    $= E_{z \sim q(z)}[log\,{P_\theta(x)}] + E_{z \sim q(z)}[log\frac{P_\theta(z|z)}{q(z)}]$ , $log\,p(x)$는 $z$와 무관한 상수이므로
    $= log\,P_\theta(x)+ E_{z \sim q(z)}[log\frac{P_\theta(z|x)}{q(z)}]$
    여기서 $q(z)$와 $P_\theta(z|x)$ 사이의 KL-divergence 는 $E_{z \sim q(z)}[log\frac{q(z)}{P_\theta(z|x)}]$ 이므로,
    $D_{KL}(q(z) || {P_\theta(z|x)}) = E_{z \sim q(z)}[log\frac{q(z)}{P_\theta(z|x)}]$ 이고, ELBO 수식을 다시 정리하면,
    $= log\,P_\theta(x) - D_{KL}(q(z) || {P_\theta(z|x)})$ 이 된다.
- 따라서, 아래와 같이 정리 할 수 있다. $$log\,p(x; \theta) = ELBO + D_{KL}(q(z)||p(z|x; \theta))$$
- **즉 $q(z)$가 $p(z|x;\theta)$에 가까워 질 수록, ELBO는 true log-likelihood에 가까워 진다고 할 수 있다.**

### Interactable Posteriors

- 여기서 $p(z|x; \theta)$가 intractable 하다면? (EM에서는 가능한 경우를 다루나, 대부분의 경우는 불가능)
- VAE에서는 decoder (the neural networks $\mu_\theta, \Sigma_\theta$) 를 "inverting" 하는 것과 대응됨.
  - $p(x | z) = N(\mu_\theta (z), \Sigma_\theta (z))$
- Interatable 한 $p(z|x; \theta)$ 에 최대한 가까운 분포를 근사하는 Variational parameter $\Phi$ 를 hidden variable로 가지는 **tractable한 $q(z; \Phi)$를** 이용해서 문제를 해결해보자
  - 예시, Gaussian with mean and covariance specfied by $\Phi$ $$q(z; \Phi)=N(\Phi_1, \Phi_2)$$
- **Variational inference**

  - pick $\Phi$ so that $q(z; \Phi)$ is as close as possible to $p(z|x;\theta)$
  - $p(z|x;\theta)$ 는 존재하는 건 알지만, 계산할 방법을 모르는 intractable optimal choice
...
- $q(z;\Phi)$를 posterior $p(z|x;\Theta)$ 에 더 잘 근사할 수록, KL-Divergence를 더 가깝게 할 수 있고, 이는 $log p(x;\theta)$를 더 잘 표현함을 의미.
...
- 전체 데이터셋에 대하여 생각해보면,
  - 각 데이터 포인트 $x^i$ 별로, True Posterior $p(z|x^i;\theta)$ 는 달라지기 때문에,
  - 각 데이터 포인트 $x^i$ 별로 각기 다른 variational parameter $\Phi^i$를 가지는 것이 한 방법이 된다.
- 따라서, $$ \underset{\theta}{max}\,l(\theta; D) \geq \underset{\theta, \Phi^1,...,\Phi^M}{max} \sum\_{x^i \in D} \mathcal{L}(x^i; \theta, \Phi^i)$$
...
- 따라서, $$E_{q(z;\Phi^i)}[log\,p(z,x^i;\theta) - log\,q(z; \Phi^i)] \approx \frac{1}{K}\sum_klog\,p(z^k, x;\theta) - log\,q(z^k;\Phi^i)$$
...
- _z가 continuous_ 할 때, 아래 식을 미분하려고 함. (r은 reward function, 앞선 Expection에 든 수식) $$E_{q(z;\Phi)}[r(z)] = \int q(z;\Phi)r(z)dz$$
...
- 여기서 Monte Carlo를 적용해보면 아래와 같다. $$E_{\epsilon\sim N(0, I)}[\nabla_\Phi(g(\epsilon;\Phi))] \approx \frac{1}{K} \sum_k \nabla_\Phi r(g(\epsilon^k;\theta))\,where\, \epsilon^1,...,\epsilon^K \sim N(0,I) $$
- ELBO 식을 다시 써보면 아래와 같다.$$\mathcal{L}(x^i; \theta, \Phi^i)= E_{q(z;\Phi^i)}[\underbrace{log\,p(z,x^i;\theta) - log\,q(z; \Phi^i)}_{r(z,\Phi)}]$$
- 수식을 보면, Reward function이 $r(z)$가 아닌 $r(z, \Phi)$임을 알 수 있는데, 이 경우에도 chain rule을 이용하여, reparameterization이 가능하다.$$E_{q(z;\Phi)}[r(z,\Phi)] = E_\epsilon[r(g(\epsilon;\Phi),\Phi)] \approx \frac{1}{K}\sum_k r(g(\epsilon^k;\Phi), \Phi)$$
...
- **Amortization**
  ...
  - 예를 들어, $q(z|x^i)$가 각기 다른 평균을 가진 가우시안이라면,
    $x^i$가 $\mu^i$로 매핑하는 하나의 Neural network $f_\lambda$를 학습한다.
- 즉, posteriors $q(z|x^i)$를 $q_\lambda(z|x)$로 근사한다
- 정확하게 다시 써본다면, **Amortized inference**는 아래와 같다.
  - learn how to map $x^i$ to a good set of parameters $\Phi^i$ via $q(z; f_\lambda(x^i))$.
- 문헌에서는, $q(z; f_\lambda(x^i))$ 는 종종 $q_\Phi(z|x)$로 쓰이기도 함.

### learning with amortized inference

- Optimize $\sum_{x^i \in D}\mathcal{L}(x^i;\theta,\Phi)$ as a function of $\theta, \Phi$ using (stochastic) gradient descent$$\mathcal{L}(x^i;\theta,\Phi)=E_{q_\Phi(z|x)}[log\,p(z,x;\theta) - log\,q_\Phi(z|x)]$$
...
$$
\begin{eqnarray}
\mathcal{L}(x^i;\theta,\Phi) & = &E_{q_\Phi(z|x)}[log\,p(z,x;\theta) - log\,q_\Phi(z|x)] \\ & = & E_{q_\Phi(z|x)}[log\,p(z,x;\theta) - log\,p(z) + log\,p(z) - log\,q_\Phi(z|x)]
\\ & = & \underbrace{E_{q_\Phi(z|x)}[log(p(x|z;\theta)]}_{reconstruction\,loss} - \underbrace{ D_{KL}(q_\Phi(z|x)||p(z))}_{KL-divergence\,loss}
\end{eqnarray}
$$

1. $q_\Phi(z|x^i)$ 의 샘플링으로, 데이터 포인트 $x^i$를 $\hat{z}$로 매핑 -> encoder
2. $p(x|\hat{z};\theta)$의 샘플링으로, $\hat{x}$를 복원. -> decoder
...
- Training Loss
  ...
  - 정규분포와 $q(z|x)$ 간의 kl divergence 로 규제.
...
- KL Divergence의 정의 $$D_{KL}​(q(z)∥p(z))=E_{q(x)}[log\frac{q(z)​}{p(z)}]$$
- 각 분포의 확률 밀도 함수$$\begin{eqnarray} q(z) &=& \frac{1}{\sqrt{2\pi\sigma^2}} exp(-\frac{{(z-\mu)}^2}{2\sigma^2}) \\ p(z) &=& \frac{1}{\sqrt{2\pi}} exp(-\frac{{z}^2}{2}) \end{eqnarray}$$
- KL Divergence에 대입하면,
  $$
  \begin{eqnarray}
  D_{KL}​(q(z)||p(z))&=&E_{q(z)}[log\frac{\frac{1}{\sqrt{2\pi\sigma^2}} exp(-\frac{{(z-\mu)}^2}{2\sigma^2})​}{\frac{1}{\sqrt{2\pi}} exp(-\frac{{z}^2}{2})}]
  \\ &=& E_{q(z)}[log(\frac{1}{\sqrt{2\pi\sigma^2}}) - \frac{{(z-\mu)}^2}{2\sigma^2} - log(\frac{1}{\sqrt{2\pi}}) + \frac{z^2}{2}]
  \\ &=& E_{q(z)}[log(\frac{1}{\sqrt{2\pi\sigma^2}}) + log(\sqrt{2\pi}) - \frac{{(z-\mu)}^2}{2\sigma^2}  + \frac{z^2}{2}]
  \\ &=& E_{q(z)}[-\frac{1}{2}log(2\pi\sigma^2) + \frac{1}{2}log(2\pi) - \frac{{(z-\mu)}^2}{2\sigma^2}  + \frac{z^2}{2}]
  \\ &=& E_{q(z)}[-\frac{1}{2}log(\sigma^2) - \frac{{(z-\mu)}^2}{2\sigma^2}  + \frac{z^2}{2}]
  \end{eqnarray}
  $$
  여기서 각각의 항에 대해 계산 한다면,
- 첫번째 항은 z와 무관한 상수이므로, $$ E\_{q(z)}[-\frac{1}{2}log(\sigma^2) ] = -\frac{1}{2}log(\sigma^2)$$
- 두번째 항에서 ${(z-\mu)}^2$의 기대 값은 $\sigma^2$ 이므로, $$ E*{q(z)}[- \frac{{(z-\mu)}^2}{2\sigma^2}] = -\frac{1}{2\sigma^2} E*{q(z)}[{(z-\mu)}^2] = -\frac{1}{2}$$
- 세번째 항은 z의 제곱에 대한 기대 값을 구하는 것 이므로, $$E_{q(z)}[\frac{z^2}{2}] = \frac{1}{2}E_{q(z)}[z^2] = \frac{1}{2}(\mu^2 +\sigma^2)$$
- 따라서 전체를 결합하면,
  $$
  \begin{eqnarray} D_{KL}​(q(z)||p(z)) &=& -\frac{1}{2}log(\sigma^2) -\frac{1}{2} + \frac{1}{2}(\mu^2 +\sigma^2)
